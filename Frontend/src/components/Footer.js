...
const Footer = () => {
  ...
  const langPath = (path) => {
    const prefix = currentLang === "fr" ? "fr" : currentLang === "ar" ? "lb" : currentLang === "it" ? "it" : ""
  ...
  return (
    <footer className="bg-[#28509E] border-t border-white/10">
      <div className="max-w-[1200px] mx-auto px-[20px] md:px-[40px] pt-[28px] md:pt-[36px] pb-[0]">
        {/* Main Footer Content */}
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-y-[24px] md:gap-y-[28px] gap-x-[32px] md:gap-x-[40px] lg:gap-x-[48px] mb-[24px] md:mb-[28px] ${currentLang === "it" ? "lg:grid-cols-3" : "lg:grid-cols-4"}`}>
          {/* Column 1: Brand */}
            ...
            <nav aria-label={t("footer.links.ariaLabel") || "Footer navigation"}>
              <div className="grid grid-cols-2 gap-x-[24px] gap-y-[8px] w-full">
                ...
                <ul className="space-y-[8px]" role="list">
                  <li>
                    <Link
                      to={currentLang === "it" ? langPath("/settori-applicazioni") : langPath("/sectors")}
                      className="font-['Open_Sans'] text-white/90 text-[14px] hover:text-[#FF8A00] transition-colors duration-200 block py-[2px] focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-[#28509E] rounded-sm"
                    >
          ...
          {/* Column 3: Contact (NAP) – hidden for Italian */}
          {currentLang !== "it" && (
            <div className="flex flex-col justify-start items-start">
        ...
        {/* Bottom Bar: locations (language-specific) → divider → copyright */}
        <div className="border-t border-white/10 pt-[20px] md:pt-[24px] pb-[8px]">
          {t("footer.locations") ? (
            <p className="text-[12px] md:text-[13px] text-white/70 font-['Open_Sans'] text-center" dir="auto">
              {t("footer.locations")}
            </p>
          ) : null}
          <div className="mt-[20px] md:mt-[24px] flex flex-col items-center gap-[12px]">
            <Link
              to={langPath("/privacy-policy")}
              className="font-['Open_Sans'] text-white/80 text-[13px] md:text-[14px] hover:text-[#FF8A00] transition-colors duration-200 underline focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-[#28509E] rounded-sm"
            >
