  ...
  const Blog = () => {
    ...
    const postsIt = useMemo(() => {
      const raw = t(`${NS}.postsIt`, { returnObjects: true })
      if (!Array.isArray(raw) || raw.length === 0) return null
      const images = [`${base}/assets/blog/blog1.png`, `${base}/assets/blog/blog2.png`, `${base}/assets/blog/blog3.png`]
      return raw.slice(0, 3).map((item, i) => ({
        id: `blog-it-${i + 1}`,
        title: item.title || "",
        excerpt: makeExcerpt(item.excerpt || ""),
        link: item.link || "",
    ...
    const posts = useMemo(() => {
      if (urlLang === "it" && postsIt && postsIt.length > 0) return postsIt
      return staticPosts.map((p) => {
        const title = t(p.titleKey)
        const excerptRaw = t(p.excerptKey)
        const excerpt = makeExcerpt(excerptRaw)
        return { ...p, title, excerpt }
      })
    }, [urlLang, postsIt, staticPosts, t])

    const visiblePosts = useMemo(() => {
      if (urlLang === "it" && postsIt && postsIt.length > 0) return postsIt
      return posts.filter((p) => p.link === "/blog-post-1" || p.link === "/blog-post-2" || p.link === "/blog-post-3")
    }, [urlLang, postsIt, posts])

    const seoHiddenPosts = useMemo(() => {
      if (urlLang === "it" && postsIt && postsIt.length > 0) return []
      return posts.filter((p) => p.link !== "/blog-post-1" && p.link !== "/blog-post-2" && p.link !== "/blog-post-3")
    }, [urlLang, postsIt, posts])
                ...
                >
                  <div className="blogCardMediaWrapper relative w-full blog-card-media-wrapper rounded-none" style={{ borderRadius: 0, overflow: "visible" }}>
                    <div className="bg-[#2B529C] text-center py-[7px] px-[20px] absolute top-[-10px] left-[-10px] z-10 rounded-none blog-card-date-badge" style={{ borderRadius: 0 }}>
                      ...
                      <p className="text-[#FFF] font-saira font-[400] text-[20px] leading-[1.1] mt-[6px]">{badge.month}</p>
                    </div>

                    <div className="relative rounded-none overflow-hidden" style={{ borderRadius: 0 }}>
                      <ImageWebp srcWebp={post.image} className="w-full object-cover rounded-none" style={{ borderRadius: 0 }} src={post.image} alt={post.title} />
                    </div>
                  </div>
