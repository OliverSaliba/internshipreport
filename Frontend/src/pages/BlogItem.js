...
const ITALIAN_BLOG_SLUGS = [
  "normative-ponteggi-2026-legge-198-2025-badge-cantiere",
  "come-scegliere-impalcature-per-restauro",
]
...
const BlogItem = () => {
  ...
  const cleanPath = useMemo(() => location.pathname.replace(/^\/(fr|lb|it)(?=\/|$)/, ""), [location.pathname])
  const postId = useMemo(() => {
    const p = cleanPath.replace(/\/$/, "")
    const m = p.match(/\/blog-post-(\d+)$/)
    return m?.[1] || ""
  }, [cleanPath])
  const slug = routeSlug && ITALIAN_BLOG_SLUGS.includes(routeSlug) ? routeSlug : null
  const cleanPathForHomeCheck = useMemo(
    () => location.pathname.replace(/^\/(fr|lb|it)(?=\/|$)/, ""),
    [location.pathname]
  )
  ...
  useEffect(() => {
    if (slug && urlLang !== "it") {
      navigate(buildPathWithLang(urlLang, "/blog"), { replace: true })
    }
  }, [slug, urlLang, navigate])
  ...
  const postConfig = useMemo(() => {
    if (slug && urlLang === "it") {
      const bySlug = t("blogItem.postsBySlug", { returnObjects: true })
      if (bySlug && typeof bySlug === "object" && bySlug[slug]) {
        const d = bySlug[slug]
        return {
          seoTitle: d.seoTitle || d.h1,
          seoDescription: d.seoDescription || "",
          canonical: d.canonical || `https://achiscaffolding.com/it/blog/${slug}`,
          h1: d.h1 || "",
          image: d.image ? (d.image.startsWith("http") ? d.image : `${base}${d.image}`) : `${base}/assets/blog/blog1.png`,
          imageAlt: d.imageAlt || d.h1,
          category: d.category || "Normative",
          readingTime: d.readingTime || "5 min read",
    ...
    const p3SectionsRaw = buildSectionsFromI18n("3")
    const p3Sections = p3SectionsRaw.map((s) => ({
      ...s,
      id: s.id.replace(/^s-/, "occ-"),
    }))
  ...
  tocEnabled: true,
  sections: p3Sections,
  conclusion: t("blogItem.posts.3.conclusion"),
  recommended: [],
  externalSources: [],
  faq: [],
  cta: {},
  disclaimer: "",
    ...
    const fallback = {
      ...
      canonical: `https://achiscaffolding.com/blog-post-${postId}`,
  ...
  if (String(postId) === "3") d.setDate(d.getDate() - 2)
  ...
  const schema = useMemo(() => {
    const origin = "https://achiscaffolding.com"
    ...
    <>
      ...
      <main id="main-content" className="w-full bg-white">
        <SEO
          ...
          ogImage={postConfig.image?.startsWith('http') ? postConfig.image : `https://achiscaffolding.com${postConfig.image}`}
          ogType="article"
        />
      {/* ---- MERGED FROM OTHER BRANCH ---- */}
      {false && (
          ...
          <div className="w-full max-w-[1200px] mx-auto px-[20px] md:px-[40px] grid grid-cols-1 lg:grid-cols-12 gap-[40px]">
            ...
            <div className="lg:col-span-8">
              ...
              <article ref={articleRef} className="prose prose-lg max-w-none">
                ...
                {postConfig.faq && postConfig.faq.length > 0 && (
                  <section className="mt-[40px] pt-[40px] border-t border-[#e0e0e0]" aria-labelledby="faq-title">
                    <h2 id="faq-title" className="font-[Rajdhani] text-[#1b3155] text-[28px] md:text-[32px] font-[700] mb-[20px] leading-[1.3]">
                      {t("blogItem.faqTitle") || "FAQ – Domande frequenti"}
                    </h2>
                    <div className="space-y-[24px]">
                      {postConfig.faq.map((item, idx) => (
                        <div key={idx}>
                          <h3 className="font-[Rajdhani] text-[#1b3155] text-[20px] md:text-[22px] font-[700] mb-[10px]">
                            {item.q}
                          </h3>
                          <p className="font-['Open_Sans'] text-[#2a2a2a] text-[16px] md:text-[17px] leading-[1.75]">
                            {item.a}
                ...
                {postConfig.disclaimer && (
                  <section className="mt-[40px] p-[24px] md:p-[32px] bg-[#f5f7fa] border border-[#e0e0e0] rounded-[0]" aria-labelledby="disclaimer-title">
                    <h2 id="disclaimer-title" className="font-[Rajdhani] text-[#1b3155] text-[22px] md:text-[26px] font-[700] mb-[12px]">
                      Aggiornamento & Note
                    </h2>
                ...
                {(postConfig.cta?.title || postConfig.cta?.text) && (
                  <section className="mt-[40px] p-[24px] md:p-[32px] bg-[#28509E] text-white rounded-[0]" aria-labelledby="cta-title">
                    <h2 id="cta-title" className="font-[Rajdhani] text-[22px] md:text-[26px] font-[700] mb-[12px]">
                      {postConfig.cta.title}
                    </h2>
                    {postConfig.cta.text && (
                      <p className="font-['Open_Sans'] text-[15px] md:text-[16px] leading-[1.7] mb-[16px] opacity-95">
                        {postConfig.cta.text}
                      </p>
                    )}
                    {postConfig.cta.href && (
                      <a
                        href={postConfig.cta.href}
                        className="inline-flex items-center justify-center px-[24px] py-[12px] bg-white text-[#28509E] font-[Rajdhani] font-[700] text-[16px] hover:bg-[#f0f0f0] transition-colors rounded-[0]"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        {postConfig.cta.buttonLabel || "Contattaci"}
                      </a>
                    )}
                  </section>
                )}
